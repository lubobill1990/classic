...
//    public $templateDirs = array();
    ...
    public function init()
    {
        ...
        //You should set path without extension
        //for example {include file="application.views.layout.main"}
        $file_extension = $this->fileExtension;

        $this->getSmarty()->default_template_handler_func =
            function ($type, $name) use ($file_extension)
            {
                $template_file_name = Yii::getPathOfAlias($name);
                $i18n_template_file_path = $template_file_name . '-' . Yii::app()->getLanguage() . $file_extension;
                if (file_exists($i18n_template_file_path)) {
                    return $i18n_template_file_path;
                }
                $file_path = $template_file_name . $file_extension;
                if (file_exists($file_path)) {
                    return $file_path;
                } else {
                    var_dump($file_path) . " not exist";
                    exit();
                    return $name;
                }
            };
    ...
    public function renderFile($context, $sourceFile, $data, $return)
    {
        ...
        if (!empty($this->_smarty_data)) {
            $data = array_merge($data, $this->_smarty_data);
        }
