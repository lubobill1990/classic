...
function calc_begin_end_week($week_info){
    preg_match("/(\\d*)-(\\d*)/", $week_info,$matches);
    return array($matches[1],$matches[2]);
}
//the function
...
function analyse_site($site){
    ...
    foreach($site_arr as $key=>$con){
        $class_info=multiexplode(array("&nbsp;"," "), $con);
        $site_arr[$key]=array();
        $site_arr[$key]['day_of_week']=calc_day_of_week($class_info[0]);
        $site_arr[$key]['time_of_day']=calc_begin_end_of_day($class_info[1]);
        $site_arr[$key]['classroom']=$class_info[2];
        $site_arr[$key]['week_info']=calc_begin_end_week($class_info[3]);
    }
    return $site_arr;
}
    ...
    public function actionGetDepartmentAndMajorInfo($username,$password){
        ...
        try{
            foreach ($matches[0] as $department){
                ...
                $dep_name=$match[2];
                $dep_more=$match[3];
                $dep=Department::model()->findByPk($dep_id);
                if(empty($dep)){
                    $dep=new Department();
                    ...
                    if(!$dep->save()){
                        throw new Exception("save department {$dep->id},{$dep->name} fail");
                    }else{
                        $dep_inserted[]=$dep;
                    }
                    ...
                    $dep->name=$dep_name;
                    if(!$dep->save()){
                        throw new Exception("update department {$dep->id},{$dep->name} fail");
                    }else{
                        $dep_updated[]=dep;
                    }
                }
                ...
                for($i=0;$i<count($majors[0]);++$i){
                    ...
                    $maj=Major::model()->findByPk($major_id);
                    if(empty($maj)){
                        $maj=new Major();
                        ...
                        if(!$maj->save()){
                            throw new Exception("insert major {$maj->id},{$maj->name} fail");
                        }else{
                            $maj_inserted[]=maj;
                        }
    ...
    public function actionExtractCourseRawData(){
        ...
        try{
            ...
            foreach($courses as $course){
                $cour=Course::model()->findByPk($course['course_id']);
                if(empty($cour)){
                    $cour=new Course();
            ...
            foreach($tmp_jw_data_chunk as $tmp_jw_data){
                $actual_class=ActualClass::model()->findByAttributes(array(
                    'course_id'=>$tmp_jw_data->course_id,
                    'term'=>$tmp_jw_data->term,
                    'grade'=>$tmp_jw_data->grade,
                    'major_id'=>$tmp_jw_data->institute_id
                ));
